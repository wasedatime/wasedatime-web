...
const Pagination = ({ threadCount, numOfThreadsPerPage, paginate} : Props) => {

  return (
    <div className="flex justify-center m-4">
      <ul className="inline-flex items-center -space-x-px">
        <li>
          <h1 className="block p-2 ml-0 rounded-full text-gray-500 hover:bg-light-main hover:text-white dark:text-gray-300 dark:hover:bg-dark-main dark:hover:text-white">
        ...
        </li>
        {
          [...Array(Math.ceil(threadCount / numOfThreadsPerPage))].map((_, pageId) => (
            <li key={pageId + 1} className='cursor-pointer' onClick={() => paginate(pageId + 1)}>
              <h1 className="px-3 py-2 rounded-full text-gray-500 hover:bg-light-main hover:text-white dark:text-gray-300 dark:hover:bg-dark-main dark:hover:text-white">{pageId + 1}</h1>
            </li>
          ))
        }
        <li>
          <h1 className="block p-2 leading-tight rounded-full text-gray-500 hover:bg-light-main hover:text-white dark:text-gray-300 dark:hover:bg-dark-main dark:hover:text-white">
