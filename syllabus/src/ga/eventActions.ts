export const gaSetLanguage = "Set Language";
export const gaChangeLanguage = "Change Language";
export const gaUserSignIn = "User Sign In";
export const gaUserSignOut = "User Sign Out";
export const gaUserSignInFailure = "User Sign In Failure";

export const gaAddCourse = "Add Course";
export const gaRemoveCourse = "Remove Course";
export const gaMakeCourseVisible = "Make Course Visible";
export const gaMakeCourseInVisible = "Make Course Invisible";
export const gaChangeCourseColor = "Change Course Color";

export const gaOpenModal = "Open modal";
export const gaCloseModal = "Close modal";
export const gaApplyFilter = "Apply Filter";
export const gaRemoveFilter = "Remove Filter";
export const gaUpdateFilter = "Update Filter";

export const gaOpenCourseDetails = "Open Course Details";
export const gaClickSyllabusLink = "Click Syllabus Link";
export const gaShareCourse = "Share Course";
export const gaClickRelatedCourse = "Click Related Course";

export const gaCreateCourseReview = "Create Course Review";
export const gaEditCourseReview = "Edit Course Review";
export const gaDeleteCourseReview = "Delete Course Review";
export const gaSwitchReviewLanguage = "Switch Review Language";

export const gaChangeSortingOption = "Change Sorting Option";

export const gaAppendActionWithLng = (action, lng) => {
  return `${action} - ${lng} user`;
};
